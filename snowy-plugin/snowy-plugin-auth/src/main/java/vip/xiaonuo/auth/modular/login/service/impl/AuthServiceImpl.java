...
import vip.xiaonuo.auth.core.pojo.SaBaseClientLoginUser;  // 保留以支持接口定义，但实际不使用
import vip.xiaonuo.auth.core.pojo.SaBaseLoginUser;
import vip.xiaonuo.auth.core.util.AuthEmailFormatUtil;
// import vip.xiaonuo.auth.core.util.StpClientLoginUserUtil;  // 边缘网关不需要C端
// import vip.xiaonuo.auth.core.util.StpClientUtil;  // 边缘网关不需要C端
...
// import vip.xiaonuo.client.ClientUserApi;  // 边缘网关不需要C端功能
...
public class AuthServiceImpl implements AuthService {
    ...
    // @Resource(name = "clientLoginUserApi")  // 边缘网关不需要C端功能
    ...
    public String getPhoneValidCode(AuthGetPhoneValidCodeParam authGetPhoneValidCodeParam, String type) {
        ...
        // 登录验证码短信消息模板编码
        String smsTemplateCode = devConfigApi.getValueByKey(SNOWY_SMS_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY);
    ...
    private void checkAllowPhoneLoginFlag(String type) {
        // 是否允许手机号登录
        String allowPhoneLoginFlag = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_ALLOW_PHONE_LOGIN_FLAG_FOR_B_KEY);
    ...
    public String getEmailValidCode(AuthGetEmailValidCodeParam authGetEmailValidCodeParam, String type) {
        ...
        // 登录验证码邮件消息模板内容
        String emailTemplateContent = devConfigApi.getValueByKey(SNOWY_EMAIL_TEMPLATE_VALID_CODE_LOGIN_FOR_B_KEY);
    ...
    private void checkAllowEmailLoginFlag(String type) {
        // 是否允许邮箱登录
        String allowEmailLoginFlag = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_ALLOW_EMAIL_LOGIN_FLAG_FOR_B_KEY);
    ...
    public String doLogin(AuthAccountPasswordLoginParam authAccountPasswordLoginParam, String type) {
        ...
        // 校验验证码
        boolean defaultCaptchaOpen = getDefaultCaptchaOpenForB();
    ...
    private int saveLoginTimes(String userAccount, String type){
        // 获取连续登录失败持续时间
        String configContinuousLoginFailDuration = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_DURATION_FOR_B_KEY);
        // 获取连续登录失败次数
        String configContinuousLoginFailTimes = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_TIMES_FOR_B_KEY);
        // 获取连续登录失败锁定时间
        String configContinuousLoginFailLockDuration = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_CONTINUOUS_LOGIN_FAIL_LOCK_DURATION_FOR_B_KEY);
        // 连续登录失败持续时间默认5分钟
    ...
    public SaBaseClientLoginUser getClientLoginUser() {
        // 边缘网关不支持C端功能
        throw new CommonException("边缘网关不支持C端功能，请使用B端管理界面");
    }
    ...
    public String doLoginByOtp(AuthOtpLoginParam authOtpLoginParam, String type) {
        ...
        // 定义验证码是否打开
        boolean defaultCaptchaOpen = this.getDefaultCaptchaOpenForB();
        // 获取验证码
        ...
        // 解密密钥
        String otpSecretKey = CommonCryptogramUtil.doSm4CbcDecrypt(sysUserExtJsonObject.getStr("otpSecretKey"));
    ...
    private void checkAllowOtpLoginFlag(String type) {
        // 是否允许动态口令登录
        String allowOtpLoginFlag = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_ALLOW_OTP_LOGIN_FLAG_FOR_B_KEY);
    ...
    private void checkAllowRegisterFlag(String type) {
        // 是否允许注册
        String allowRegisterFlag = devConfigApi.getValueByKey(SNOWY_SYS_DEFAULT_ALLOW_REGISTER_FLAG_FOR_B_KEY);

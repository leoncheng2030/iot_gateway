...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public void exportUser(SysUserExportParam sysUserExportParam, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            // 优化：使用分页查询，避免一次性加载所有数据到内存
            int pageSize = 500; // 每批次导出500条
            int pageNum = 1;
            ExcelWriter excelWriter = null;
            WriteSheet writeSheet = null;
            
            try {
                excelWriter = EasyExcel.write(tempFile.getPath(), SysUserExportResult.class)
                        // 自定义样式
                ...
                writeSheet = EasyExcel.writerSheet("用户信息").build();
                ...
                while (true) {
                    ...
                    List<SysUserExportResult> sysUserExportResultList = sysUserList.stream()
                            .map(sysUser -> {
                                ...
                                // 优化：仅在头像存在且数据量较小时转换图片，避免OOM
                                if (ObjectUtil.isNotEmpty(sysUser.getAvatar()) && totalCount <= 1000) {
                                    try {
                    ...
                    // 写入当前批次数据
                    excelWriter.write(sysUserExportResultList, writeSheet);
